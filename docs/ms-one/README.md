...
Этот раздел описывает план реализации чтения сущностей OneNote из спецификации **[MS-ONE]** (см. `language-agnostic-plan/ms-one_spec_structure.txt`) на Python, используя уже готовый слой чтения контейнера **[MS-ONESTORE]** (`src/onestore/*`) и документацию в `docs/ms-onestore/*`.
