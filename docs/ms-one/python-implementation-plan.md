...
> Примечание: большая часть описанного v1 уже реализована в `src/ms_one/` (и зеркально в `src/aspose/note/_internal/ms_one/`).
> Этот документ остаётся полезным как карта спецификации, но отдельные пункты могут быть уже закрыты реализацией.
...
Готово когда:
- `python -m unittest` проходит локально (без сетевых зависимостей).
