...
- `ObjectSpaceObjectPropSet` парсится структурно, а `PropertySet` значения декодируются отдельным шагом (см. ниже).
- объектный padding после `PropertySet` не выделяется отдельным полем: при декодировании допустимы `0..7` нулевых байт в конце `rgData`.
...
- для ссылочных типов (`0x08..0x0D`) значения возвращаются как `CompactID` или `tuple[CompactID, ...]` из соответствующих потоков (без разрешения в GUID на этом уровне).
- поддержаны вложенные наборы свойств: `0x11` (nested PropertySet) и `0x10` (prtArrayOfPropertyValues, декодируется как массив вложенных PropertySet).
