...
### 2.1 Инструменты
- `pytest` как основной раннер.
- Snapshot-тесты (на ваш выбор):
  - либо `pytest` + собственная JSON-схема “summary” (предпочтительно, без доп. зависимостей),
  - либо `pytest-approvaltests`/`snapshottest` (если ок доп. зависимости).
...
- Единая pytest-фикстура `simpletable_path` (ищет файл в корне репозитория).
- Фикстура `simpletable_bytes` (читает bytes один раз, даёт `BinaryReader`).
...
**Готово, если**
- `pytest -k simpletable` стабильно проходит и ловит регрессии.
